...
def main(args):
    ...
    for level in range(common_level+3, int(np.log2(target_res))-1):
        ...
        dataloader = DataLoader(super_set, batch_size=batch_size[level],
                                shuffle=True, num_workers=0)

        if level != common_level+1:
            face_encoder = Encoder(
                int(cur_res/2), common_level).to(device)
            face_decoders = []
            for i in range(num_person):
                face_decoders.append(
                    Decoder(int(cur_res/2), common_level).to(device))

            Logger = LogModel(
                int(cur_res/2), person_num=num_person, saving_path=args.model_folder)
            _ = Logger.load_model(face_encoder, face_decoders)

            face_encoder.next_level()
            for i in range(num_person):
                face_decoders[i].next_level()
        else:
        ...
        visdom_visualize = visdom.Visdom(env="disney")
        last_count = 0
        count = 0
        loss_count = 0

        for epoch in tqdm(range(num_epoch[level])):
            last_count = count
            if epoch < num_epoch[level]:
                cur_alpha = epoch/num_epoch[level]
            else:
                cur_alpha = 1
        ...
        if level != common_level+1:
            face_encoder.update_RGB()
            for i in range(num_person):
                face_decoders[i].update_RGB()

        Logger.log_model(num_epoch[level], face_encoder.state_dict(), [
                         decoder_i.state_dict() for decoder_i in face_decoders])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--batch_size', type=str,
                        default="[64, 64, 64, 32, 32, 16, 8, 4, 2]")
    parser.add_argument('-e', '--epoch', type=str,
                        default="[30, 30, 30, 75, 100, 125, 150, 200, 100]")
    parser.add_argument('-lr', '--learning_rate', type=str,
                        default="[1e-4, 1e-4, 1e-4, 5e-5, 5e-5, 2e-5, 2e-5, 2e-5, 5e-5]")
